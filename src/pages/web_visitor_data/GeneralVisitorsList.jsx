...
const GeneralVisitorsList = () => {
  ...
  const [open, setOpen] = useState(""); // ✅ fix 1 — open state

  const { generalVisitors, loading } = useSelector(
    (state) => state.generalVisitors,
  );

  const handle = (value) => setOpen(value); // ✅ fix 2 — handle function
  ...
  const columns = [
    { label: "Registration ID", accessor: "regId.no" },
    {
      ...
      render: (value, row) => (
        <Link
          to={`/webVisitorData/generalVisitorDetails/${row._id}`} // use row.id
